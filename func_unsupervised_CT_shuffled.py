...
"""
Created on Sat Mar 2 18:03:24 2024

@author: Eric Li, Hannah Choi
"""
"""
This code contains functions needed for mapping CT clusters to FF/FB by 
maximizing the global hierarchy score of the shuffled cortico-thalamic connectivity data, 
and is called in "run_TCCT_shuffled.py".
"""
...
def assign_hierarchy_levels(df, i_shuffle, j):
    """Returns global hierarchy scores with (hgc)/without (hg) CT confidence 
    (= equal # of FF/FB bias)"""
    ...
    df_cortex=pd.read_excel(r'./Output/shuffled/CCshuffled_conf_iter'+str(i_shuffle)+'.xlsx')
    ...
    levels_s = df.groupby('source')[10].mean()
    ##################################################################
...
def fit_CT(df, i_shuffle, parallel=True, n_procs=-1):
