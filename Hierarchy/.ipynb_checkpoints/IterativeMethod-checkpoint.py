...
def iterativeCC(dfi,dfV,n_iter):
    ...
    for i_iter in range(1,n_iter+1):
        ...
        hrc_iter.iloc[:,i_iter+1]=hrc_iter.iloc[:,i_iter+1]-np.mean(hrc_iter.iloc[:,i_iter+1])
        hr_iter.iloc[:,i_iter+1]=hr_iter.iloc[:,i_iter+1]-np.mean(hr_iter.iloc[:,i_iter+1])
...
def iterativeTCCT(dfiC, dfVC, dfiT, dfVT, n_iter):
    ...
    dfi = pd.concat([dfiT, dfiC])
    dfV = pd.concat([dfVT, dfVC])
...
# In[]: Cortico-basal ganglia iterations

def iterativeCB(dfiCT, dfVCT, dfiB, dfVB, n_iter):
    
    dfi = pd.concat([dfiB, dfiCT])
    dfV = pd.concat([dfVB, dfVCT])
